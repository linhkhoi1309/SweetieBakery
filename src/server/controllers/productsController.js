...
export async function createProduct(req, res) {
  const { name, price, description, category, stock, images, priceSale } =
    req.body || {};
  ...
  if (!trimmedName || !trimmedDescription) {
    return res
      .status(400)
      .json({ success: false, message: "Tên và mô tả sản phẩm là bắt buộc" });
  }
  ...
  if (numericPrice === undefined || numericStock === undefined) {
    return res
      .status(400)
      .json({ success: false, message: "Giá và số lượng tồn kho phải là số" });
  }
  ...
  if (!normalizedImages.length) {
    return res
      .status(400)
      .json({ success: false, message: "Ít nhất một ảnh hợp lệ là bắt buộc" });
  }
...
export async function updateProduct(req, res) {
  ...
  try {
    ...
    if (name?.trim()) {
      ...
      product.slug = await slugGenerator(name.trim());
    }
    ...
    if (priceSale !== undefined) {
      ...
      if (numericPriceSale === undefined) {
        return res
          .status(400)
          .json({ success: false, message: "Giá khuyến mãi không hợp lệ" });
      }
    ...
    if (stock !== undefined) {
      ...
      if (numericStock === undefined) {
        return res
          .status(400)
          .json({ success: false, message: "Số lượng tồn kho không hợp lệ" });
      }
    ...
    if (images !== undefined) {
      ...
      if (images && !normalizedImages.length) {
        return res
          .status(400)
          .json({ success: false, message: "Danh sách ảnh không hợp lệ" });
      }
      product.images = normalizedImages;
    }

    if (rest.sold !== undefined)
      product.sold = parseNumber(rest.sold, product.sold);
    if (rest.rating !== undefined)
      product.rating = parseNumber(rest.rating, product.rating);
    if (rest.numReviews !== undefined)
      product.numReviews = parseNumber(rest.numReviews, product.numReviews);
