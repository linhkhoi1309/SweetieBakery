...
export const applyCoupon = async (req, res) => {
    try {
        ...
        if (!orderValue) {
            return res.status(400).json({ success: false, message: 'Cần cung cấp giá trị đơn hàng.' });
        }
        ...
        if (!coupon.isActive) {
            return res.status(400).json({ success: false, message: 'Mã giảm giá hiện đang bị khóa.' });
        }
        ...
        if (now < coupon.startDate) {
            return res.status(400).json({ success: false, message: 'Chương trình khuyến mãi chưa bắt đầu.' });
        }
        ...
        if (orderValue < coupon.minOrderValue) {
            return res.status(400).json({ 
                success: false, 
                message: `Đơn hàng phải từ ${coupon.minOrderValue.toLocaleString('vi-VN')}đ mới được áp dụng.` 
            });
        }
...
export const getAllCoupons = async (req, res) => {
    try {
        ...
        const formattedCoupons = coupons.map(coupon => {
            ...
            else if (coupon.usedCount >= coupon.usageLimit) status = 'sold_out'; // Hết lượt
        ...
        res.status(200).json({ success: true, count: coupons.length, data: formattedCoupons });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
